...
Mouse Magnet Behaviour in Wheel:

Computes the distance from the mouse to its base position:
let d = dist(mouseX, mouseY, this.baseX, this.baseY);


Defines an influence radius:
let influenceRadius = this.baseRadius * 4;


If the wheel is within this radius, a simple strength factor is computed:
let strength = 1 - d / influenceRadius;  // from 0 (far) to 1 (very close)


The wheel is moved a small step towards the cursor:
this.x += (mouseX - this.x) * 0.08 * strength;
this.y += (mouseY - this.y) * 0.08 * strength;


The rotation speed increases near the mouse:
this.rotationSpeed = this.baseRotationSpeed * (1 + 2 * strength);


If the wheel is outside the radius or the magnet is disabled (magnetEnabled == false), it eases back:
